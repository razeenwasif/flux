...
const Downloads: Component = () => {
  ...
  return (
    <div style={{ display: "contents" }}>
      <button classList={{ "icon-btn": true, active: open() }} title="Downloads" onClick={() => { setOpen((v) => !v); if (!open()) refresh(); }}>
        ⬇
        <Show when={active() > 0}>
        ...
        <div class="glass popover footer-pop">
          <div class="shields-row">
            ...
            <Show when={items().length > 0}>
              <button class="dl-clear" onClick={() => void downloadsClear().then(refresh)}>Clear</button>
            </Show>
          </div>
          <Show when={items().length > 0} fallback={<div class="start-empty" style={{ padding: "4px 8px 8px" }}>No downloads yet.</div>}>
            <For each={items()}>
              {(d) => (
                <div class="dl-row">
                  <div class="dl-head">
                    <span class="dl-name" title={d.path || d.url}>{d.filename || d.url}</span>
                    <span class="dl-actions">
                      <Show when={d.state === "in_progress"}>
                        <button class="find-nav" title="Pause" onClick={() => void downloadPause(d.id)}>⏸</button>
                        <button class="find-nav" title="Cancel" onClick={() => void downloadCancel(d.id)}>✕</button>
                      </Show>
                      <Show when={d.state === "paused"}>
                        <button class="find-nav" title="Resume" onClick={() => void downloadResume(d.id)}>▶</button>
                        <button class="find-nav" title="Cancel" onClick={() => void downloadCancel(d.id)}>✕</button>
                      </Show>
                      <Show when={d.state === "completed"}>
                        <button class="find-nav" title="Open" onClick={() => void downloadOpen(d.id)}>↗</button>
                        <button class="find-nav" title="Show in folder" onClick={() => void downloadReveal(d.id)}>📂</button>
                      </Show>
                  ...
                  <Show when={d.state === "in_progress" || d.state === "paused"}>
                    <div class="dl-bar"><div class="dl-fill" classList={{ indet: pct(d) === null }} style={pct(d) !== null ? { width: `${pct(d)}%` } : undefined} /></div>
                  </Show>
                  <div class="dl-sub">
                    {d.state === "completed" ? fmtBytes(d.received)
                      : d.state === "interrupted" ? "Failed"
                      : d.state === "paused" ? `Paused · ${fmtBytes(d.received)}`
                      : d.total > 0 ? `${fmtBytes(d.received)} / ${fmtBytes(d.total)}` : fmtBytes(d.received)}
                  </div>
                </div>
              )}
            </For>
