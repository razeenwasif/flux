...
| 12 | ◐ | Generate TS types from Rust structs via `specta` (kill the manual `ipc.ts` mirror). _Done:_ `specta::Type` on **21 IPC structs** → `bindings.gen.ts` (`bindings.rs::generate_ts`, regen `FLUX_WRITE_BINDINGS=1 cargo test -p flux-core bindings`), drift-gated in CI; `ipc.ts` re-exports them all — `state.rs` (TabKind/TabMeta/TabGroup/TabFolder/ClusterTag/Workspace/Container/WebPanel/AgentStatus) + Bookmark/Feed/FeedItem/PwaApp/HistoryEntry/SavedTab/SavedSession/ProcInfo/SysStats/SpeedResult/ArchiveMeta. _Remaining:_ the smaller/misc structs (Omni*, Shields*, vault, extensions, downloads, files, macros, boosts) still hand-written; `ArchiveEntry` needs a wire/persist split (carries embeddings); **command-wrapper codegen (tauri-specta) is blocked** — it needs the specta v2 / tauri-specta v2 **pre-release** stack (Tauri v2 incompat with the stable specta v1 we use), deferred until runtime-verifiable. |
...
| 71 | P2 | **Start page** — shipped: search hero, clock + real weather, recent, **editable** speed dial (persisted), quick actions, flowing wave, **feed headlines (#72), top sites (from history), scratchpad (persisted), calendar + world clocks**. Remaining: drag-reorder + show/hide widgets, agent summaries, custom backgrounds | locked-down elsewhere |
