...
impl<R: Runtime> WebviewManager<R> {
  ...
  fn prepare_pending_webview<M: Manager<R>>(
    ...
    // Flux patch: browser documents are websites, not the enclosing app.
    // Keep the framework marker on the trusted shell only. IPC bootstrap and
    // capability checks below are unchanged for every webview.
    if !label.starts_with("tab-") && !label.starts_with("panel-") {
      all_initialization_scripts.push(main_frame_script(
        "Object.defineProperty(window, 'isTauri', { value: true });".to_owned(),
      ));
    }
