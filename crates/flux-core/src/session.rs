...
mod tests {
    ...
    fn tab(id: TabId, pinned: bool) -> TabMeta {
        TabMeta { id, kind: TabKind::Browser, url: format!("https://{id}.test"), title: String::new(), pinned, cluster: None }
    }

    #[test]
    fn round_trips() {
        let path = std::env::temp_dir().join(format!("flux-sess-{}-a.json", std::process::id()));
        save(&path, &Session { tabs: vec![tab(2, true), tab(5, false)], active: 5, next_id: 6 });
    ...
    fn restore_repopulates_and_bumps_next_id() {
        ...
        save(&path, &Session { tabs: vec![tab(7, true)], active: 7, next_id: 2 });
