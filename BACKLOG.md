...
Status: backend implemented + unit-tested for all eight (`cache.rs`, `shields.rs`,
`prefetch.rs`, `leanmode.rs`, `hibernate.rs`, `flux-agent`); remaining work is
frontend wiring + the engine-gated pieces, noted per item.

| # | P | Item |
|---|---|---|
| 99 | ◐ | **Hot/cold filter tiering** (extends #57, arXiv 1810.09160). _Done:_ the tokenized engine is already fast, so the win is **not re-running it** — verdicts are memoized per `(url, source-host, type)` in a bounded TTL/LRU decision cache (`shields.rs`, cleared on rule rebuild), and firing rules are recorded into a **hot set** (`shields_hot_rules`); `shields_status` now reports cache hit-% + distinct rules fired. _Remaining:_ surface the hot set in the shields UI; optionally persist it across sessions. |
...
| 103 | ◐ | **Confidence-gated predictive prefetch** (ties #70, #45, arXiv 1906.00877). _Done:_ `prefetch.rs` — per-origin Markov chain with LFU decay, confidence + min-sample thresholds, ranked `prefetch_hints`, and a `prefetch_set_pressure` gate that silences speculation under memory pressure (6 tests). _Remaining:_ chrome calls `prefetch_record` on navigation + issues the preconnect from `prefetch_hints`; wire the pressure gate to the resource governor (#70). |
...
| 105 | ◐ | **Per-site lean mode** (perf, arXiv 2106.08948, 2308.16729). _Done:_ `leanmode.rs` — opt-in per host; when on, a curated performance filter list (`assets/lean-filters.txt`: tag managers, analytics, A/B, session replay, chat/social widgets) blocks heavy third-party scripts on top of shields, wired into the request interceptor (4 tests). _Remaining:_ the **dynamic per-function dead-JS elimination** (coverage trace → empty-body/lazy-load with a screenshot-diff oracle) needs a webview coverage hook the native engines don't expose through Tauri; plus a lean-mode toggle in the UI. |
| 106 | ◐ | **Smarter hibernation eviction + energy framing** (ties #45, #70, arXiv 1202.5539, 2205.11399). _Done:_ `hibernate_rank` orders sleep candidates **Belady-style** — least likely to be needed next, discounting idle time by the #103 prediction — with a "kept" flag for predicted-return tabs (3 tests). _Remaining:_ the memory-pressure path calls `hibernate_rank` instead of plain LRU; surface dark-mode/shields/hibernation as measured **battery** features; rematerialization policy. |
