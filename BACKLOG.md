...
| 57 | P1 | **Built-in content blocker** (engine + WebView2 interceptor landed via #91, blocking verified on Windows): `flux-filter` (Brave's `adblock`) + a bundled curated starter list block the major ad/tracker networks at the request level. **Remaining: fetch/cache full EasyList/uBO + user lists, element-hiding cosmetic filters (injected CSS), and the per-site shields UI.** | Brave, Vivaldi, Opera |
