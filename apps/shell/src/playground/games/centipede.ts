...
export default function centipede(ctx: GameCtx): GameHandle {
  ...
  for (let i = 0; i < 32; i++) {
    const r = 1 + ((Math.random() * (BAND - 1)) | 0);
  ...
  const k = keys(
    (key) => {
      ...
      else if (key === "ArrowUp" || key === "w") up = true;
    ...
    (key) => {
      ...
      else if (key === "ArrowUp" || key === "w") up = false;
  ...
  function tickBugs() {
    for (const bug of bugs) {
      ...
      let nc = head.c + bug.dir,
        nr = head.r;
      if (nc < 0 || nc >= COLS || (mush[nr]?.[nc] ?? 0) > 0) {
        nr = Math.min(ROWS - 1, head.r + 1);
        bug.dir = -bug.dir;
        nc = head.c;
        if (head.r >= ROWS - 1) nr = head.r - 1;
      }
  ...
  function shoot(bx: number, by: number) {
    ...
    // Mushroom hit?
    if (br >= 0 && br < ROWS && bc >= 0 && bc < COLS && mush[br]![bc]! > 0) {
      mush[br]![bc]!--;
      if (mush[br]![bc] === 0) {
        score += 1;
        ctx.setScore(score);
  ...
  function step(dt: number) {
    ...
    if (invuln <= 0) {
      const pc = Math.floor(px / CELL),
        pr = Math.floor(py / CELL);
      for (const bug of bugs)
        if (bug.cells.some((s) => s.c === pc && s.r === pr)) {
          hitPlayer();
          break;
        }
    }
  ...
  function draw() {
    ...
    g.beginPath();
    g.moveTo(0, BAND * CELL);
    g.lineTo(W, BAND * CELL);
    g.stroke();
    for (let r = 0; r < ROWS; r++)
      for (let c = 0; c < COLS; c++)
        if (mush[r]![c]! > 0) {
          const hp = mush[r]![c]!;
          g.fillStyle = hp === 3 ? "#5dff8f" : hp === 2 ? "#8a7dff" : "#ff8a3d";
          g.fillRect(c * CELL + 6, r * CELL + 6, CELL - 12, CELL - 12);
        }
    for (const bug of bugs)
      bug.cells.forEach((s, i) => {
        g.shadowBlur = 8;
        g.shadowColor = "#ff4d9d";
        g.fillStyle = i === 0 ? "#ff9ac6" : "#ff4d9d";
        g.beginPath();
        g.arc(s.c * CELL + CELL / 2, s.r * CELL + CELL / 2, CELL / 2 - 4, 0, Math.PI * 2);
        g.fill();
        g.shadowBlur = 0;
      });
    g.fillStyle = "#ffe14d";
