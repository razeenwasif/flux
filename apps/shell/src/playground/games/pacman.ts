...
export default function pacman(ctx: GameCtx): GameHandle {
  ...
  const pac: Ent & { want: [number, number] | null; mouth: number } = {
    c: 9,
    r: 15,
    dir: null,
    prog: 0,
  ...
  const ghosts: Ent[] = [
    { c: 7, r: 9, dir: [1, 0], prog: 0 },
    { c: 9, r: 9, dir: [-1, 0], prog: 0 },
    { c: 11, r: 9, dir: [1, 0], prog: 0 },
  ];
  let fright = 0;
  let combo = 0;
  let score = 0;
  ...
  function px(e: Ent) {
    return OX + ((e.dir ? e.c + e.dir[0] * e.prog : e.c) + 0.5) * CELL;
  }
  function py(e: Ent) {
    return OY + ((e.dir ? e.r + e.dir[1] * e.prog : e.r) + 0.5) * CELL;
  }
  ...
  function draw() {
    ...
    for (let r = 0; r < ROWS; r++)
      for (let c = 0; c < COLS; c++) {
        ...
        if (ch === "#") {
          ...
          g.fillStyle = "#ffe14d";
          g.beginPath();
          g.arc(x + CELL / 2, y + CELL / 2, 6, 0, Math.PI * 2);
          g.fill();
          g.shadowBlur = 0;
        }
      }
    ...
    ghosts.forEach((gh, i) => {
      g.fillStyle =
        fright > 0
          ? fright < 1600 && Math.floor(fright / 200) % 2 === 0
            ? "#e8e6f4"
            : "#3a5bff"
          : GCOLORS[i]!;
      const x = px(gh),
        y = py(gh);
      g.beginPath();
      g.arc(x, y, CELL / 2 - 2, Math.PI, 0);
      g.lineTo(x + CELL / 2 - 2, y + CELL / 2 - 2);
      g.lineTo(x - CELL / 2 + 2, y + CELL / 2 - 2);
      g.closePath();
      g.fill();
      g.fillStyle = "#fff";
      g.beginPath();
      g.arc(x - 4, y - 2, 2.5, 0, Math.PI * 2);
      g.arc(x + 4, y - 2, 2.5, 0, Math.PI * 2);
      g.fill();
    });
    // Pac.
    ...
    g.beginPath();
    g.moveTo(0, 0);
    g.arc(0, 0, CELL / 2 - 1, a, Math.PI * 2 - a);
    g.closePath();
    g.fill();
    g.restore();
    g.shadowBlur = 0;
