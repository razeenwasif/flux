...
fn invoked_commands(files: &[PathBuf]) -> BTreeSet<String> {
    ...
    for f in files {
        ...
        while let Some(pos) = src[search..].find("invoke") {
            ...
            if !cmd.is_empty() && cmd.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || c == '_') {
...
fn every_invoked_command_is_registered() {
    ...
    let lib_rs = std::fs::read_to_string(root.join("crates/flux-core/src/lib.rs")).expect("read lib.rs");
