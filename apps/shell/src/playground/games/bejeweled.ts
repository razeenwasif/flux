...
export default function bejeweled(ctx: GameCtx): GameHandle {
  ...
  for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) {
    do { grid[r]![c] = rc(); }
    while ((c >= 2 && grid[r]![c] === grid[r]![c - 1] && grid[r]![c] === grid[r]![c - 2]) || (r >= 2 && grid[r]![c] === grid[r - 1]![c] && grid[r]![c] === grid[r - 2]![c]));
  }

  function matches(): Set<number> {
    const m = new Set<number>();
    for (let r = 0; r < N; r++) for (let c = 0; c < N - 2; c++) { const v = grid[r]![c]; if (v && v === grid[r]![c + 1] && v === grid[r]![c + 2]) { m.add(r * N + c); m.add(r * N + c + 1); m.add(r * N + c + 2); } }
    for (let c = 0; c < N; c++) for (let r = 0; r < N - 2; r++) { const v = grid[r]![c]; if (v && v === grid[r + 1]![c] && v === grid[r + 2]![c]) { m.add(r * N + c); m.add((r + 1) * N + c); m.add((r + 2) * N + c); } }
    return m;
  }
  ...
  function anyMove(): boolean {
    for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) {
      for (const [nr, nc] of [[r, c + 1], [r + 1, c]] as [number, number][]) {
  ...
  const onDown = (e: MouseEvent) => {
    ...
    if (!adj) { sel = [r, c]; return; }
    swap(sel, [r, c]);
    if (matches().size) { resolve(); if (!anyMove()) over = true; }
  ...
  function draw() {
    ...
    g.fillStyle = "#0f0c1a"; g.fillRect(OX, OY, N * CELL, N * CELL);
    for (let r = 0; r < N; r++) for (let c = 0; c < N; c++) {
      const v = grid[r]![c]; if (!v) continue;
      const x = OX + c * CELL, y = OY + r * CELL;
      g.shadowBlur = 6; g.shadowColor = GEMS[v - 1]!; g.fillStyle = GEMS[v - 1]!;
      g.beginPath(); g.arc(x + CELL / 2, y + CELL / 2, CELL / 2 - 5, 0, Math.PI * 2); g.fill(); g.shadowBlur = 0;
    }
    if (sel) { g.strokeStyle = "#fff"; g.lineWidth = 2; g.strokeRect(OX + sel[1] * CELL + 2, OY + sel[0] * CELL + 2, CELL - 4, CELL - 4); g.lineWidth = 1; }
  }

  const l = loop(() => {
    draw();
    if (over && !reported) { reported = true; saveHighScore("bejeweled", score); ctx.onGameOver(score); }
  });
