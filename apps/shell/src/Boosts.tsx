...
const Boosts: Component<{ initialOpen?: boolean }> = (props) => {
  ...
  const Row = (p: { b: Boost; showHost?: boolean }) => (
    <div class="boost-row-wrap">
      <div class="boost-row">
        <button
          classList={{ "boost-toggle": true, on: p.b.enabled }}
          title={p.b.enabled ? "Enabled — click to disable" : "Disabled"}
          onClick={() => toggle(p.b)}
        >
          {p.b.enabled ? "●" : "○"}
        </button>
        <span class="boost-name" title={p.b.css}>
          {p.b.name || "boost"}
          {p.showHost ? <span class="boost-host-tag"> · {p.b.host}</span> : null}
        </span>
        <button
          class="boost-del"
          title="Edit CSS"
          onClick={() => (editId() === p.b.id ? setEditId(null) : startEdit(p.b))}
        >
          ✎
        </button>
        <button class="boost-del" title="Delete boost" onClick={() => remove(p.b)}>
          ✕
        </button>
      </div>
      <Show when={editId() === p.b.id}>
        <textarea
          ...
          value={editCss()}
          spellcheck={false}
          onInput={(e) => setEditCss(e.currentTarget.value)}
        />
        <div class="boost-edit-actions">
          <button class="boost-apply" disabled={busy()} onClick={() => void saveEdit(p.b)}>
            Save CSS
          </button>
          <button class="boost-del" onClick={() => setEditId(null)}>
            Cancel
          </button>
        </div>
      </Show>
  ...
  return (
    <div style={{ display: "contents" }}>
      <button
        ...
        title="Boosts — customize this site with AI or CSS"
        onClick={() => setOpen((v) => !v)}
      >
        ✨
      </button>
      <Show when={open()}>
        ...
        <div class="glass popover shields-pop footer-pop">
          <div class="shields-row">
            ...
            <button
              classList={{ "boost-mode-tab": true, on: allSites() }}
              title="Manage boosts on every site"
          ...
          <Show
            ...
            fallback={
              <Show
                ...
                fallback={<div class="boost-empty">No boosts saved yet.</div>}
              >
                <div class="ctx-label">
                  {allList().length} boost{allList().length === 1 ? "" : "s"} across{" "}
                  {new Set(allList().map((b) => b.host)).size} site
                  {new Set(allList().map((b) => b.host)).size === 1 ? "" : "s"}
                </div>
            ...
            <Show when={host()} fallback={<div class="boost-empty">Open a web page to boost it.</div>}>
              <div class="shields-row">
                ...
                <div class="boost-mode">
                  <button
                    classList={{ "boost-mode-tab": true, on: mode() === "ai" }}
                    onClick={() => setMode("ai")}
                  >
                    AI
                  </button>
                  <button
                    classList={{ "boost-mode-tab": true, on: mode() === "css" }}
                    onClick={() => setMode("css")}
                  >
                    CSS
                  </button>
              ...
              <textarea
                ...
                placeholder={
                  mode() === "ai"
                    ? "Describe a change — e.g. hide the cookie banner, dark mode, widen the article"
                    : "Write CSS — e.g.  .cookie-banner { display: none !important; }"
                }
                rows={mode() === "css" ? 4 : 2}
                value={instruction()}
                disabled={busy()}
              ...
              >
                {busy()
                  ? "✨ Saving…"
                  : mode() === "ai"
                    ? "✨ Apply (the agent writes the CSS)"
                    : "✨ Apply CSS"}
              </button>
              <Show when={error()}>
                <div class="boost-error">{error()}</div>
              </Show>
