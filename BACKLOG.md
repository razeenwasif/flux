...
| 58 | P1 | **HTTPS-only mode** ✅ + **cookie clear** ✅ (http→https upgrade via the #91 interceptor + per-site allow-HTTP; clear-site/clear-all via WebView2 `CookieManager`; Shields-popover controls; verified vs msvc). **Remaining: clear-on-close, third-party-cookie blocking, per-site permission prompts (camera/mic/geo), and a downgrade interstitial.** | Brave, Firefox |
