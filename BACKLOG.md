...
| 35 | ◐ | **Built-in PDF viewer** (done): PDFs open in a Flux-owned PDF.js viewer (`flux://pdf`) — continuous pages, zoom, page count, download — cross-engine (WebKitGTK has no native viewer). `.pdf` addresses, ⌘K, and `target="_blank"` PDF links route here; bytes fetched by `pdf_fetch` (server-side, CORS-free, base64, 32 MB cap; http(s)/file://); PDF.js lazy-loaded so the chrome bundle stays lean. _Remaining:_ annotation + form-fill; in-page link clicks to PDFs still use the engine's native viewer on WebView2 (would need request interception to reroute). | all |
