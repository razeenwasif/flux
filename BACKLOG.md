...
| # | P | Item | Inspiration |
|---|---|---|---|
| 91 | P1 | **Request-interception layer** (ADR 0007; engine + policy landed): the `flux-filter` matching engine + `shields` policy (global/per-site, stats) are done and tested. **Remaining: the native per-webview interceptor** that calls `ShieldsState::should_block` — WebView2 `WebResourceRequested` (`AddWebResourceRequestedFilter` + a `CreateWebResourceResponse(403)` on block) first, WebKitGTK follow-up. Reused for #58's http→https rewrite. | enabling primitive |
| 57 | P1 | **Built-in content blocker** (engine landed via #91): `flux-filter` (Brave's `adblock`) + a bundled curated starter list block the major ad/tracker networks. **Remaining: the native interceptor wiring (#91), fetch/cache full EasyList/uBO + user lists, element-hiding cosmetic filters (injected CSS), and the per-site shields UI.** | Brave, Vivaldi, Opera |
