...
| 61 | 🚧 | **Password manager + autofill + passkeys** (ADR 0009; in progress). _Done:_ `flux-vault` crate — credential model, AES-256-GCM seal/open (Zeroizing plaintext), conservative host matching, **Proton Pass JSON importer**, unit-tested. _Remaining:_ OS-keychain data key (Windows Credential Manager/DPAPI, macOS Keychain, libsecret via `keyring`) + flux-core commands + persistence; **autofill** via JS injection (same-origin, user-initiated); a vault/import UI; Chrome/1Password/Bitwarden importers; optional master password. Passkeys/WebAuthn handled by the native webview. _Note:_ Proton ships only a WebExtension (can't run in native webviews) + no public API → import is a snapshot (re-export to resync). | all |
