...
export default function bubble(ctx: GameCtx): GameHandle {
  ...
  function rc() {
    return 1 + ((Math.random() * COLORS.length) | 0);
  }
  ...
  const onDown = () => {
    if (!fly && !over) fly = { x: SX, y: SY, vx: Math.cos(aim) * 0.7, vy: Math.sin(aim) * 0.7, col: cur };
  };
  ...
  const neighbors = (r: number, c: number): [number, number][] =>
    [
      ...
      [r, c - 1],
      [r, c + 1],
    ].filter(([rr, cc]) => rr! >= 0 && rr! < ROWS && cc! >= 0 && cc! < COLS) as [number, number][];
  ...
  function group(r: number, c: number, col: number): [number, number][] {
    ...
    while (stack.length) {
      ...
      for (const [nr, nc] of neighbors(cr, cc))
        if (!seen.has(nr * COLS + nc) && grid[nr]![nc] === col) {
          seen.add(nr * COLS + nc);
          stack.push([nr, nc]);
        }
    }
  ...
  function settle(fx: number, fy: number, col: number) {
    let r = Math.round((fy - OY - D / 2) / D),
      c = Math.round((fx - OX - D / 2) / D);
    ...
    if (grid[r]![c] !== 0) {
      ...
      empt.sort(
        (a, b) => Math.hypot(cx(a[1]) - fx, cy(a[0]) - fy) - Math.hypot(cx(b[1]) - fx, cy(b[0]) - fy),
      );
      if (empt.length) {
        r = empt[0]![0];
        c = empt[0]![1];
    ...
    if (grp.length >= 3) {
      for (const [gr, gc] of grp) grid[gr]![gc] = 0;
      score += grp.length * 10;
      dropFloating();
      ctx.setScore(score);
    }
  ...
  function dropFloating() {
    ...
    while (stack.length) {
      ...
      for (const [nr, nc] of neighbors(r, c))
        if (grid[nr]![nc] && !anchored.has(nr * COLS + nc)) {
          anchored.add(nr * COLS + nc);
          stack.push([nr, nc]);
        }
    }
    let dropped = 0;
    for (let r = 0; r < ROWS; r++)
      for (let c = 0; c < COLS; c++)
        if (grid[r]![c] && !anchored.has(r * COLS + c)) {
          grid[r]![c] = 0;
          dropped++;
        }
    if (dropped) {
  ...
  function step(dt: number) {
    ...
    if (!hit)
      for (let r = 0; r < ROWS && !hit; r++)
        for (let c = 0; c < COLS; c++)
          if (grid[r]![c] && Math.hypot(fly.x - cx(c), fly.y - cy(r)) < D - 4) {
            hit = true;
            break;
          }
    if (hit) {
  ...
  function draw() {
    g.fillStyle = "#0c0a15";
    g.fillRect(0, 0, W, H);
    g.strokeStyle = "rgba(255,77,109,0.35)";
    g.setLineDash([6, 6]);
    g.beginPath();
    g.moveTo(OX, cy(MAXROW) + D / 2);
    g.lineTo(OX + COLS * D, cy(MAXROW) + D / 2);
    g.stroke();
    g.setLineDash([]);
    for (let r = 0; r < ROWS; r++)
      for (let c = 0; c < COLS; c++) if (grid[r]![c]) bub(cx(c), cy(r), grid[r]![c]!);
    // Aim guide.
    g.strokeStyle = "rgba(255,255,255,0.25)";
    g.beginPath();
    g.moveTo(SX, SY);
    g.lineTo(SX + Math.cos(aim) * 90, SY + Math.sin(aim) * 90);
    g.stroke();
    bub(SX, SY, cur);
    bub(SX + 34, SY + 6, next);
    if (fly) bub(fly.x, fly.y, fly.col);
  }
  function bub(x: number, y: number, col: number) {
    g.shadowBlur = 6;
    g.shadowColor = COLORS[col - 1]!;
    g.fillStyle = COLORS[col - 1]!;
    g.beginPath();
