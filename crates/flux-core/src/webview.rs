  ...
  for (const h of {json}) {{
    ...
    mod script_join_tests {
        ...
        fn a_script_ending_without_a_newline_still_closes_its_block() {
            ...
            let blob =
                join_page_scripts(&[("a", "var x=1; // trailing comment"), ("b", "var y=2;")]);
            assert!(
...
#[cfg(test)]
mod capture_js_tests {
    // Included directly rather than via the const, which lives inside a
    // platform-gated module — the asset is what these assertions are about.
    const CAPTURE_JS: &str = include_str!("../assets/capture.js");

    /// The observer's options decide whether the agent sees the page at all, and
    /// getting them wrong fails silently — the snapshot is simply stale forever.
    ///
    /// The bug this pins: a page that renders "loading…" and then *reveals* its
    /// content by flipping a class produces no `childList` and no
    /// `characterData` mutation. `innerText` respects CSS visibility, so the
    /// text genuinely changed while nothing the observer watched had. Measured
    /// in a real engine before and after — attributes fire nothing without this.
    #[test]
    fn the_observer_watches_visibility_attributes() {
        assert!(
            CAPTURE_JS.contains("attributes: true"),
            "a class/hidden flip that reveals content must be observed"
        );
        for attr in ["class", "style", "hidden", "aria-hidden"] {
            assert!(
                CAPTURE_JS.contains(&format!("\"{attr}\"")),
                "{attr} changes what innerText returns, so it must be in the filter"
            );
        }
        // Filtered, not blanket: every attribute would let an SPA's hover and
        // focus classes restart the debounce continuously, which is the mutation
        // storm the debounce exists to prevent.
        assert!(
            CAPTURE_JS.contains("attributeFilter"),
            "an unfiltered attribute observer re-publishes on every hover"
        );
    }

    /// Rust calls this by name to freshen a snapshot before the agent reads it;
    /// renaming it would break that silently, since the eval is guarded and a
    /// missing function is a no-op rather than an error.
    #[test]
    fn recapture_is_exposed_for_the_on_demand_refresh() {
        assert!(CAPTURE_JS.contains("recapture"), "dom::refresh calls this");
        // …and it must bypass the debounce, or "refresh then read" would race
        // the 400 ms timer it was meant to sidestep.
        assert!(
