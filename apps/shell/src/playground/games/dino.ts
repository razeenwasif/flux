...
interface Obstacle { x: number; kind: "cactus" | "bird"; w: number; h: number; y: number }
...
export default function dino(ctx: GameCtx): GameHandle {
  ...
  function spawn() {
    if (Math.random() < 0.7) obstacles.push({ x: W, kind: "cactus", w: 16 + ((Math.random() * 3) | 0) * 10, h: 34, y: GROUND - 34 });
  ...
  function step(dt: number) {
    ...
    spawnIn -= dt;
    if (spawnIn <= 0) { spawn(); spawnIn = (620 + Math.random() * 520) / (speed / 0.36); }
    for (const o of obstacles) o.x -= speed * dt;
    obstacles = obstacles.filter((o) => o.x + o.w > -4);

    const d = dinoBox();
    for (const o of obstacles) {
      if (d.x < o.x + o.w - 6 && d.x + d.w - 6 > o.x && d.y < o.y + o.h - 4 && d.y + d.h > o.y + 4) { over = true; break; }
    }
  }
  ...
  function draw() {
    ...
    const d = dinoBox();
    g.shadowBlur = 12; g.shadowColor = "#2ff3ff"; g.fillStyle = "#aefcff";
    g.fillRect(d.x, d.y, d.w, d.h);
    g.shadowBlur = 0; g.fillStyle = "#0c0a15";
    g.fillRect(d.x + d.w - 12, d.y + 5, 4, 4); // eye
    if (onGround()) { const up = Math.floor(legT / 90) % 2 === 0; g.fillStyle = "#2ff3ff"; g.fillRect(d.x + 4, d.y + d.h, 6, up ? 6 : 3); g.fillRect(d.x + d.w - 14, d.y + d.h, 6, up ? 3 : 6); }

    for (const o of obstacles) {
      g.shadowBlur = 8;
      if (o.kind === "cactus") { g.shadowColor = "#5dff8f"; g.fillStyle = "#3ddc78"; g.fillRect(o.x, o.y, o.w, o.h); }
      else { g.shadowColor = "#ff8a3d"; g.fillStyle = "#ff8a3d"; const flap = Math.floor(legT / 130) % 2 === 0; g.fillRect(o.x, o.y + 8, o.w, 8); g.fillRect(o.x + 6, flap ? o.y : o.y + 14, o.w - 14, 6); }
    }
    g.shadowBlur = 0;
  }
  ...
  const l = loop((dt) => {
    if (!over) step(dt);
    draw();
    if (over && !reported) { reported = true; saveHighScore("dino", Math.floor(score)); ctx.onGameOver(Math.floor(score)); }
  });
