...
const MusicBubble: Component = () => {
  ...
  let fails = 0;
  const poll = async () => {
    try { setSt(await spotifyState()); setReachable(true); fails = 0; }
    catch { if (++fails >= 2) setReachable(false); }
  };
  ...
  const leave = () => { clearTimeout(leaveT); leaveT = window.setTimeout(() => setHovering(false), 280); };
  ...
  const togglePlay = () => run(playing() ? "⏸ Paused" : "▶ Playing", playing() ? spotifyPause() : spotifyResume());
  ...
  const setVol = (v: number) => { setSt((s) => (s ? { ...s, volume: v } : s)); void spotifyVolume(v).catch(() => {}); };
  const play = (pl: SpotifyPlaylist) => { setMenuOpen(false); run(`▶ ${pl.name}`, spotifyPlayContext(pl.uri), 800); };
  const launch = () => run("Starting AudioPulse…", spotifyLaunch(), 4000);

  const pct = () => { const s = st(); return s && s.duration_ms > 0 ? Math.min(100, (s.progress_ms / s.duration_ms) * 100) : 0; };
        ...
        <div class="music-body">
          <div class="music-track">
            <Show when={st()?.track} fallback={<span class="music-dim">{reachable() ? "—" : "off"}</span>}>
              <span class="music-title" title={`${st()!.track} — ${st()!.artist}`}>{st()!.track}</span>
              <span class="music-artist" title={st()!.artist}>{st()!.artist}</span>
            </Show>
          </div>

          <div class="music-prog"><div class="music-prog-fill" style={{ width: `${pct()}%` }} /></div>
          <div class="music-times"><span>{fmt(st()?.progress_ms ?? 0)}</span></div>
          ...
          <input class="music-vol" type="range" min="0" max="100" value={st()?.volume ?? 50} title="Volume" onInput={(e) => setVol(Number(e.currentTarget.value))} />

          <button class="music-btn music-menu-btn" title="Playlists" onClick={() => { setMenuOpen((v) => !v); if (!playlists().length) loadPlaylists(); }}>≡</button>

          <Show when={st() && !st()!.has_device}>
            <button class="music-launch" title="No active device — start AudioPulse" onClick={launch}>Start ▶</button>
          </Show>
        </div>
      </div>
      ...
      <Show when={menuOpen()}>
        <Portal>
          ...
          <div class="music-menu">
            ...
            <For each={playlists()} fallback={<div class="music-dim" style={{ padding: "10px 12px" }}>No playlists found.</div>}>
              {(p) => (
