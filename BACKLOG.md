...
| # | P | Item | Inspiration |
|---|---|---|---|
| 91 | P1 | **Request-interception layer** (ADR 0007): engine + policy + the **WebView2 interceptor** are wired (`with_webview` → `WebResourceRequested` → `403` on block; COM compile-verified vs msvc, runtime pending a Windows smoke test). **Remaining: the WebKitGTK interceptor** (web-process extension) for the Linux/dev backend. Reused for #58's http→https rewrite. | enabling primitive |
| 57 | P1 | **Built-in content blocker** (engine + WebView2 interceptor landed via #91): `flux-filter` (Brave's `adblock`) + a bundled curated starter list block the major ad/tracker networks at the request level. **Remaining: runtime-verify the WebView2 block on Windows, fetch/cache full EasyList/uBO + user lists, element-hiding cosmetic filters (injected CSS), and the per-site shields UI.** | Brave, Vivaldi, Opera |
