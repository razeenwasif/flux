...
| 58 | P1 | **HTTPS-only mode** ✅ (http→https upgrade via the #91 interceptor + per-site allow-HTTP + Shields toggle; verified vs msvc). **Remaining: downgrade interstitial** (site has no HTTPS), **granular cookie controls** (WebView2 `CookieManager` / WebKitGTK `WebKitCookieManager`), **per-site permission prompts**, and **clear-on-close**. | Brave, Firefox |
