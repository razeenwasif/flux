...
| 59 | P1 | **Multi-account containers / profiles**. _Ephemeral done:_ **private/incognito tabs** — "🕶 Private tab" (new-tab picker + ⌘K) opens a tab whose webview uses an in-memory session (`WebviewBuilder::incognito`), never recorded in history/Omni and never persisted across restart; violet rail/tint marks it. _Remaining:_ **named persistent containers** — isolated cookie/storage jars via `WebviewBuilder::data_directory` per container (the API exists in Tauri 2.11), with a per-tab container picker (like workspaces). | Firefox, Safari profiles |
