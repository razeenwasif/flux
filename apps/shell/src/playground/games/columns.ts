...
export default function columns(ctx: GameCtx): GameHandle {
  ...
  function rc() { return 1 + ((Math.random() * GEMS.length) | 0); }

  function canFall() { return row + 2 + 1 < ROWS && board[row + 3]![col] === 0; }
  function cellsFree(c: number) { return board[row]![c] === 0 && board[row + 1]![c] === 0 && board[row + 2]![c] === 0; }
    ...
    for (;;) {
      ...
      for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) {
        ...
        for (const [dr, dc] of dirs) {
          const run = [r * COLS + c];
          let rr = r + dr!, cc = c + dc!;
          while (rr >= 0 && rr < ROWS && cc >= 0 && cc < COLS && board[rr]![cc] === v) { run.push(rr * COLS + cc); rr += dr!; cc += dc!; }
  ...
  const k = keys((key) => {
    if (over) return;
    if (key === "ArrowLeft" || key === "a") { if (col > 0 && cellsFree(col - 1)) col--; }
    else if (key === "ArrowRight" || key === "d") { if (col < COLS - 1 && cellsFree(col + 1)) col++; }
    else if (key === "ArrowUp" || key === "w") colr = [colr[2]!, colr[0]!, colr[1]!];
    else if (key === "ArrowDown" || key === "s") { if (canFall()) { row++; acc = 0; } else lock(); }
    else if (key === " " || key === "Spacebar") { while (canFall()) row++; lock(); acc = 0; }
  });
  ...
  function draw() {
    ...
    g.fillStyle = "#07060d"; g.fillRect(OX, OY, COLS * CELL, ROWS * CELL);
    g.strokeStyle = "rgba(255,255,255,0.05)";
    for (let c = 0; c <= COLS; c++) { g.beginPath(); g.moveTo(OX + c * CELL, OY); g.lineTo(OX + c * CELL, OY + ROWS * CELL); g.stroke(); }
    for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) if (board[r]![c]) gem(board[r]![c]!, OX + c * CELL, OY + r * CELL, false);
  ...
  const l = loop((dt) => {
    if (!over) {
      acc += dt;
      if (acc >= stepMs) { acc = 0; if (canFall()) row++; else lock(); stepMs = Math.max(180, 520 - score / 40); }
    }
    draw();
    if (over && !reported) { reported = true; saveHighScore("columns", score); ctx.onGameOver(score); }
  });
