    ...
    return [
      {
        ...
        num: s.live_docs.toLocaleString(),
        sub: `${s.total_docs.toLocaleString()} on disk`,
      },
      ...
      {
        label: "Embeddings",
        num: s.embedded ? `on · ${s.embedder_kind} ${s.embedder_dim}d` : "off",
        sub: "semantic / hybrid",
        sm: true,
      },
      {
        label: "ANN (HNSW)",
        num: s.ann ? `${s.ann_vectors.toLocaleString()} vec` : "—",
        sub: s.ann ? "sub-linear recall" : "brute-force",
        sm: true,
      },
      {
        label: "Avg length",
        num: `${Math.round(s.avg_body_len)}`,
        sub: `body · ${Math.round(s.avg_title_len)} title`,
        sm: true,
      },
    ];
  });
  ...
  return (
    <div class="omni">
      <header class="omni-head">
        <span class="omni-brand">
          <span class="omni-spark">✦</span> Omni <span class="omni-sub">index dashboard</span>
        </span>
        <span style={{ flex: 1 }} />
        <div class="omni-viewtoggle">
          <button classList={{ on: view() === "stats" }} onClick={() => setView("stats")}>
            Stats
          </button>
          <button
            classList={{ on: view() === "graph" }}
            onClick={() => setView("graph")}
            title="Visualise the index as a semantic graph"
          >
            Graph
          </button>
        </div>
        ...
        <Show when={!error()} fallback={<span class="omni-status off">offline</span>}>
          <span class="omni-status live">
            <span class="omni-dot" /> live · {REFRESH_MS / 1000}s
          </span>
        </Show>
      </header>
            ...
            <>
              ...
              <section class="omni-panel">
                <div class="omni-panel-h">
                  Segments{" "}
                  <span class="omni-note">
                    {s().segment_sizes.length} · largest {segMax().toLocaleString()} docs
                  </span>
                </div>
                  ...
                  <For each={s().segment_sizes}>
                    {(x, i) => (
                      <div class="omni-segrow">
                        ...
                        <span
                          class="omni-segbar"
                          style={{ width: `${Math.max(2, (x.total / segMax()) * 100)}%` }}
                        >
                          <span
                            class="omni-segfill"
                            style={{ width: `${x.total ? (x.live / x.total) * 100 : 0}%` }}
                          />
                        </span>
                        <span class="omni-segn">
                          {x.live.toLocaleString()}
                          {x.live !== x.total ? ` / ${x.total.toLocaleString()}` : ""}
                        </span>
                      </div>
                    )}
                  </For>
              ...
              <section class="omni-panel">
                <div class="omni-panel-h">
                  Essential sites{" "}
                  <span class="omni-note">
                    type <code>!key</code> in search to jump
                  </span>
                </div>
              ...
              <section class="omni-panel">
                ...
                <Show
                  when={topDocs().length > 0}
                  fallback={<div class="omni-empty sm">No PageRank yet (no link graph).</div>}
                >
