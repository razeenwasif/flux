...
const TrackerGraphView: Component = () => {
  ...
  const onMove = (e: PointerEvent) => {
    ...
    if (dragNode) {
      ...
      if (n !== hoverNode) {
        hoverNode = n;
        setHover(n ? `${n.id} · ${n.requests} req${n.blocked ? ` · ${n.blocked} blocked` : ""}` : null);
        draw();
      }
    }
    lastX = px;
  ...
  const onUp = (e: PointerEvent) => {
    if (dragNode && !moved) {
      setTrackerGraphOpen(false);
      void openTab("browser", `https://${dragNode.id}`);
    }
    dragNode = null;
    panning = false;
    try {
  ...
  const load = async () => {
    try {
      ...
      setStats({
        nodes: nodes.length,
        edges: edges.length,
        trackers: g.nodes.filter((n) => n.kind === "third" && n.blocked > 0).length,
      });
      alpha = 1;
      kick(1);
    } catch {
  ...
  const clearAll = async () => {
    await trackerClear().catch(() => {});
    nodes = [];
    edges = [];
    setStats({ nodes: 0, edges: 0, trackers: 0 });
    setEmpty(true);
    draw();
  };
  ...
  return (
    <Portal>
      <div
        class="trk-backdrop"
        onClick={() => setTrackerGraphOpen(false)}
        onKeyDown={(e) => {
          if (e.key === "Escape") setTrackerGraphOpen(false);
        }}
      >
        <div class="trk-panel glass" onClick={(e) => e.stopPropagation()}>
          <div class="trk-head">
            <span class="trk-title">🕸 Tracker graph</span>
            <span class="trk-stats">
              {stats().nodes} domains · {stats().edges} links · {stats().trackers} blocked trackers
            </span>
            <span class="trk-legend">
              <span class="trk-key">
                <i style={{ background: "#7b61ff" }} /> site
              </span>
              <span class="trk-key">
                <i style={{ background: "#2ff3ff" }} /> 3rd-party
              </span>
              <span class="trk-key">
                <i style={{ background: "#f85149" }} /> tracker (blocked)
              </span>
            </span>
            <button class="trk-btn" onClick={() => void load()}>
              ↻
            </button>
            <button class="trk-btn" onClick={() => void clearAll()}>
              Clear
            </button>
            <button class="trk-btn" onClick={() => setTrackerGraphOpen(false)}>
              ✕
            </button>
          </div>
          <div class="trk-canvas-wrap" ref={wrap}>
            ...
            <Show when={empty()}>
              <div class="trk-empty">
                No third-party requests recorded yet.
                <br />
                Browse a few sites and reopen — the graph fills as you go.
              </div>
            </Show>
            <Show when={hover()}>
              <div class="trk-tip">{hover()}</div>
            </Show>
          </div>
        </div>
      </div>
