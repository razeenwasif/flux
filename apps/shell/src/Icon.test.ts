...
describe("icon set", () => {
  ...
  it("draws something for every icon", () => {
    for (const n of names) {
      const a = ICONS[n] as { d: string[]; c?: unknown[]; dot?: unknown[] };
  ...
  it("keeps every drawing inside the 24-unit grid", () => {
    ...
    for (const n of names) {
      for (const [cx, cy, r] of ICONS[n].c ?? []) {
  ...
  it("has an icon for every page the launcher lists", () => {
    ...
    for (const n of [
      "notebook", "trail", "whiteboard", "scribe", "sessions", "archive", "feeds",
      "history", "bookmarks", "tasks", "resources", "speedtest", "omni", "apps",
      "passwords", "sync", "settings", "terminal", "agent", "calendar", "mail",
      "shields", "boosts", "macros", "note", "panels", "archived", "extensions",
      "workspaces",
    ]) {
