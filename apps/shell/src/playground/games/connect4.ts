...
export default function connect4(ctx: GameCtx): GameHandle {
  ...
  function score(b: Board): number {
    ...
    const win = (cells: number[]) => {
      const a = cells.filter((x) => x === 2).length,
        p = cells.filter((x) => x === 1).length;
      if (a && p) return 0;
      if (a === 3) return 50;
      if (a === 2) return 8;
      if (a === 1) return 1;
  ...
  function draw() {
    g.fillStyle = "#0c0a15";
    g.fillRect(0, 0, W, H);
    if (phase === "play" && hoverCol >= 0 && hoverCol < COLS && board[0]![hoverCol] === 0) {
      g.fillStyle = "rgba(255,225,77,0.12)";
      g.fillRect(OX + hoverCol * CELL, OY - 10, CELL, ROWS * CELL + 10);
    }
    g.fillStyle = "#241f4a";
    ...
    for (let r = 0; r < ROWS; r++)
      for (let c = 0; c < COLS; c++) {
        const v = board[r]![c];
        g.beginPath();
        g.arc(OX + c * CELL + CELL / 2, OY + r * CELL + CELL / 2, CELL / 2 - 6, 0, Math.PI * 2);
        if (v) {
          g.shadowBlur = 10;
          g.shadowColor = v === 1 ? "#ffe14d" : "#ff4d6d";
          g.fillStyle = v === 1 ? "#ffe14d" : "#ff4d6d";
        } else {
          g.shadowBlur = 0;
          g.fillStyle = "#0c0a15";
        }
