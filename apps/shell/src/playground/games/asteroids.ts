...
interface Rock { x: number; y: number; vx: number; vy: number; r: number; size: number; spin: number; a: number; verts: number[] }
...
export default function asteroids(ctx: GameCtx): GameHandle {
  ...
  let left = false, right = false, thrust = false;
  let fire = false, fireCool = 0;
  let rocks: Rock[] = [];
  let bullets: Bullet[] = [];
  let score = 0, lives = 3, wave = 1, invuln = 0;
  ...
  function makeRock(x: number, y: number, size: number): Rock {
    ...
    return { x, y, vx: Math.cos(dir) * speed, vy: Math.sin(dir) * speed, r: RSIZE[size]!, size, spin: (Math.random() - 0.5) * 0.003, a: 0, verts };
  }
  function spawnWave() {
    rocks = [];
    const n = 3 + wave;
  ...
  function step(dt: number) {
    ...
    if (thrust) { ship.vx += Math.cos(ship.a) * 0.0004 * dt; ship.vy += Math.sin(ship.a) * 0.0004 * dt; }
    ...
    if (fire && fireCool <= 0) {
      fireCool = 260;
      bullets.push({ x: ship.x + Math.cos(ship.a) * 14, y: ship.y + Math.sin(ship.a) * 14, vx: ship.vx + Math.cos(ship.a) * 0.5, vy: ship.vy + Math.sin(ship.a) * 0.5, life: 800 });
    }
    for (const b of bullets) { b.x = wrap(b.x + b.vx * dt, W); b.y = wrap(b.y + b.vy * dt, H); b.life -= dt; }
    bullets = bullets.filter((b) => b.life > 0);

    for (const r of rocks) { r.x = wrap(r.x + r.vx * dt, W); r.y = wrap(r.y + r.vy * dt, H); r.a += r.spin * dt; }
    ...
    for (const r of rocks) {
      ...
      if (!hit) fresh.push(r);
      else if (r.size > 1) { fresh.push(makeRock(r.x, r.y, r.size - 1), makeRock(r.x, r.y, r.size - 1)); }
    }
    rocks = fresh;
    bullets = bullets.filter((b) => b.life > 0);
    ...
    // Ship vs rock.
    if (invuln <= 0) {
      for (const r of rocks) if (Math.hypot(ship.x - r.x, ship.y - r.y) < r.r + 10) { hitShip(); break; }
    }
    if (rocks.length === 0) { wave++; spawnWave(); }
  }
  ...
  function draw() {
    ...
    for (const r of rocks) {
      ...
      for (let i = 0; i < r.verts.length; i++) {
        ...
        const px = r.x + Math.cos(ang) * rad, py = r.y + Math.sin(ang) * rad;
    ...
    g.fillStyle = "#ffe14d";
    for (const b of bullets) { g.beginPath(); g.arc(b.x, b.y, 2.5, 0, Math.PI * 2); g.fill(); }
    ...
    if (!(invuln > 0 && Math.floor(invuln / 120) % 2 === 0)) {
      ...
      g.closePath();
      g.stroke();
      if (thrust) { g.beginPath(); g.moveTo(-6, -4); g.lineTo(-16, 0); g.lineTo(-6, 4); g.stroke(); }
      g.restore();
    }
    g.shadowBlur = 0;
