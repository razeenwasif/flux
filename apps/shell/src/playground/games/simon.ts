...
export default function simon(ctx: GameCtx): GameHandle {
  ...
  function flash(i: number, ms = 320) {
    lit = i;
    tone(pads[i]!.freq, ms);
    window.setTimeout(() => {
      if (lit === i) lit = -1;
  ...
  function padPath(p: Pad) {
    const start =
      p.x < 0 && p.y < 0 ? Math.PI : p.x > 0 && p.y < 0 ? -Math.PI / 2 : p.x < 0 && p.y > 0 ? Math.PI / 2 : 0;
    g.beginPath();
